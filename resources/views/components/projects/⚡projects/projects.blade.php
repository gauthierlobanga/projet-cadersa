    ...
    <section class="px-5 py-8 xs:px-8 md:p-10 mx-auto max-w-7xl lg:px-12">
        <div class="mb-16 max-w-3xl" x-data="cspState" x-intersect="shown = true">
            @if ($this->about->project_hero_badge)
                <p class="inline-flex items-center gap-2 rounded-full bg-emerald-100 px-4 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 transition-all duration-300 delay-100 ease-out"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr items-start gap-7 no-animate"
                ...
                <a wire:navigate href="{{ route('projects.show', $project) }}"
                    ...
                    <div
                        ...
                        @if ($project->hasMedia('cover'))
                            <img loading="eager" decoding="async" src="{{ $project->getFirstMediaUrl('cover', 'card') }}"
                                alt="{{ $project->title }}"
            ...
            <nav class="mt-14 flex justify-center">
                <div x-data="buttonAnimation" class="rounded-full bg-emerald-200 dark:bg-emerald-900">
                    <a href="{{ route('projects.index') }}" wire:navigate
                        ...
                        <span data-text class="grow">Voir tous les projets</span>
                        <div data-icon
