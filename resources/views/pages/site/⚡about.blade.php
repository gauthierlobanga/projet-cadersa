...
<div>
    ...
    <section class="relative isolate overflow-hidden">
        ...
        <div class="absolute inset-0">
            <img src="{{ $this->about->hero_image_url ? Storage::url($this->about->hero_image_url) : asset('images/hero.png') }}"
                alt="Paysage rural de la RDC" class="h-full w-full object-cover" />
        ...
        <div class="pointer-events-none absolute inset-0">
            <div class="absolute left-0 top-0 h-100 w-152 rounded-full bg-emerald-500/10 blur-[160px] transform-gpu"></div>
            <div class="absolute right-0 bottom-0 h-128 w-lg rounded-full bg-teal-500/10 blur-[160px] transform-gpu"></div>
        </div>
        ...
        <div class="relative mx-auto flex min-h-[90svh] max-w-7xl items-center px-6 pt-26 pb-24 lg:px-8">
            <div class="max-w-4xl">
                ...
                {{-- Title --}}
                <h1 class="mt-8 max-w-4xl text-5xl font-semibold tracking-tight text-white md:text-6xl lg:text-7xl">
                    Construire des
                    <span
                ...
                {{-- Subtitle --}}
                <p class="mt-8 max-w-2xl text-lg leading-8 text-zinc-300 md:text-xl">
                ...
                {{-- Stats --}}
                <div class="mt-16 grid grid-cols-2 gap-8 border-t border-white/10 pt-8 md:grid-cols-4">
                    <div>
                        <div class="text-3xl font-semibold text-white">15+</div>
                        <p class="mt-2 text-sm text-zinc-400">années d'expérience</p>
                    </div>
                    <div>
                        <div class="text-3xl font-semibold text-white">120+</div>
                        <p class="mt-2 text-sm text-zinc-400">projets réalisés</p>
                    </div>
                    <div>
                        <div class="text-3xl font-semibold text-white">50K+</div>
                        <p class="mt-2 text-sm text-zinc-400">bénéficiaires</p>
                    </div>
                    <div>
                        <div class="text-3xl font-semibold text-white">26</div>
                        <p class="mt-2 text-sm text-zinc-400">territoires couverts</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section id="about" class="relative overflow-hidden bg-white py-28 dark:bg-zinc-950">
        <div class="pointer-events-none absolute inset-0">
            <div class="absolute left-0 top-0 h-128 w-lg rounded-full bg-emerald-500/5 blur-[140px] transform-gpu"></div>
            <div class="absolute right-0 bottom-0 h-112 w-md rounded-full bg-teal-500/5 blur-[140px] transform-gpu"></div>
        </div>
        ...
        <div class="relative mx-auto max-w-7xl px-6 lg:px-8">
            ...
            <div x-cloak class="mb-16 max-w-3xl" x-data="{ shown: false }" x-intersect="shown = true">
                ...
                @if ($this->about->about_text)
                    <div class="mt-8 text-lg leading-8 text-zinc-600 dark:text-zinc-400 transition-all duration-700 delay-200 ease-out"
                        :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
            ...
            {{-- Grille image + vision/mission avec animations individuelles --}}
            <div class="grid gap-16 lg:grid-cols-2 lg:items-start">
                {{-- Colonne Image : animée depuis la gauche --}}
                <div x-cloak x-data="{ shown: false }" x-intersect="shown = true"
                    ...
                    <div
                        class="overflow-hidden rounded-4xl border border-zinc-200/60 shadow-2xl shadow-zinc-200/20 dark:border-white/10 dark:shadow-black/20">
                        <img src="{{ $this->about->about_image_url ? Storage::url($this->about->about_image_url) : asset('images/agriculture.png') }}"
                            alt="Agriculture moderne en RDC"
                            class="aspect-4/5 w-full object-cover transition duration-700 hover:scale-105">
                    </div>

                    {{-- Carte flottante --}}
                    <div
                        class="absolute -bottom-8 left-8 right-8 rounded-2xl border border-zinc-200/70 bg-white/90 px-6 py-5 backdrop-blur-xl dark:border-white/10 dark:bg-zinc-900/80 shadow-lg shadow-zinc-200/10 dark:shadow-black/10">
                        <div class="flex items-baseline gap-1">
                            <span class="text-3xl font-bold text-zinc-900 dark:text-white">15+</span>
                            <span class="text-sm font-medium text-zinc-500 dark:text-zinc-400">ans</span>
                        </div>
                        <p class="mt-1 text-sm leading-relaxed text-zinc-600 dark:text-zinc-300">
                            D'engagement continu pour le développement rural et la sécurité alimentaire.
                        </p>
                    </div>
                </div>

                {{-- Colonne Vision & Mission : animée depuis la droite, avec cascade verticale --}}
                <div x-cloak class="space-y-12" x-data="{ shown: false }" x-intersect="shown = true">
                    {{-- Vision --}}
                    @if ($this->about->vision_text)
                        <div class="border-l-2 border-emerald-500 pl-6 transition-colors hover:border-emerald-400"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <h3 class="text-xl font-semibold text-zinc-900 dark:text-white">Notre vision</h3>
                            </div>
                            <div class="mt-4 leading-8 text-zinc-600 dark:text-zinc-400">
                    ...
                    @if ($this->about->mission_text)
                        <div class="border-l-2 border-teal-500 pl-6 transition-colors hover:border-teal-400"
                            x-data="{ shown: false }" x-intersect="shown = true"
                            :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'"
                            class="transition-all duration-700 delay-100 ease-out">
                            <div class="flex items-center gap-3">
                                <div
                                    class="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-500/10 text-teal-600">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <h3 class="text-xl font-semibold text-zinc-900 dark:text-white">Notre mission</h3>
                            </div>
                            <div class="mt-4 leading-8 text-zinc-600 dark:text-zinc-400">
                    ...
                    {{-- Boutons d'action --}}
                    <div class="flex flex-wrap gap-4 pt-8 transition-all duration-700 delay-300 ease-out"
                        :class="shown ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'">
                        <a href="{{ route('about') }}"
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
                ...
                @foreach ($stats as $index => $stat)
                    <div x-cloak x-data="{ shown: false, count: 0 }" x-intersect="shown = true"
                        ...
                        <div
                            class="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-8 backdrop-blur-md transition-all duration-300 hover:-translate-y-1 hover:border-emerald-500/30 hover:bg-white/10 hover:shadow-2xl hover:shadow-emerald-500/10">
                            <div
