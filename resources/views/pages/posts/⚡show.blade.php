...
new #[Layout('layouts::main')] class extends Component {
    ...
    public function mount(Post $post)
    {
        ...
        // Compteur de vues avec protection par session
...
<div class="min-h-screen bg-white dark:bg-zinc-950">
    ...
    <section
        ...
        @if ($post->hasMedia('featured'))
            <img fetchpriority="high" loading="eager" src="{{ $post->getFirstMediaUrl('featured') }}" alt="{{ $post->title }}"
        ...
        <div class="relative z-10 mx-auto w-full max-w-5xl px-4 sm:px-6 lg:px-8" x-data="cspState"
            ...
            <div class="flex flex-col items-center text-center lg:items-start lg:text-left">
                @if ($post->categories->isNotEmpty())
                    <span
                        class="mb-6 sm:mb-8 inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-4 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-white backdrop-blur-md transition-all duration-300"
                ...
                <h1 class="max-w-4xl text-balance text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-extrabold leading-tight tracking-tight text-white font-serif transition-all duration-300 delay-100"
                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                    {{ $post->title }}
                </h1>

                <div class="mt-6 sm:mt-10 flex flex-wrap items-center justify-center gap-x-4 sm:gap-x-8 gap-y-2 sm:gap-y-3 text-sm sm:text-base text-zinc-200 lg:justify-start transition-all duration-300 delay-200"
                    :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'">
                    ...
                    @if ($post->user)
                        <span class="flex items-center gap-2.5">
                            <img loading="eager" decoding="async" src="{{ $post->user->avatar_url }}" alt="{{ $post->user->name }}"
        ...
        <div class="grid gap-8 sm:gap-12 lg:grid-cols-12 lg:gap-16">
            ...
            <div class="lg:col-span-8" x-cloak x-data="cspState" x-intersect.once="shown = true"
                ...
                @if (count($this->galleryImages) > 0)
                    <div x-cloak x-data="cspState" x-intersect.once="shown = true"
                        :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'"
                        class="mt-10 sm:mt-16 transition-all duration-300 ease-out">
            ...
            <aside class="lg:col-span-4">
                <div class="sticky top-28 flex flex-col gap-6 sm:gap-8">
                        ...
                        <div x-data="cspState" @mouseenter="hover = true" @mouseleave="hover = false"
                            ...
                            <div class="relative bg-white/50 px-6 pb-8 pt-8 dark:bg-zinc-900/50">
                                <div class="relative mx-auto mb-5 flex h-20 w-20 items-center justify-center">
                                    ...
                                    <img loading="eager" decoding="async" src="{{ $post->user->avatar_url }}" alt="{{ $post->user->name }}"
                                        class="relative h-20 w-20 rounded-full object-cover ring-1 ring-white dark:ring-zinc-800">
                                </div>
                                <div class="text-center">
                                    ...
                                    <p class="mt-4 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">
                                        Expert et contributeur passionné par le développement web moderne.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {{-- Documents PDF --}}
                    @if ($this->hasPdf)
                        <div class="rounded-2xl border border-zinc-200/50 bg-zinc-50/50 p-5 backdrop-blur-lg shadow-sm dark:border-zinc-800/50 dark:bg-zinc-900/30">
                            <h3 class="mb-4 flex items-center gap-2.5 text-sm font-semibold text-zinc-900 dark:text-white">
                                <svg class="h-4 w-4 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                Documents joints
                            </h3>
                            <div class="flex flex-col gap-2">
                                @foreach ($this->pdfs as $pdf)
                                    <a href="{{ $pdf['url'] }}" target="_blank" class="group flex items-center gap-2.5 border border-zinc-200/50 bg-white/50 p-2.5 transition-all duration-300 hover:-translate-y-0.5 hover:border-emerald-300/50 hover:bg-emerald-50/30 dark:border-zinc-800/50 dark:bg-zinc-900/50 dark:hover:border-emerald-700/50 dark:hover:bg-emerald-900/10">
                                        ...
                                        <span class="flex-1 text-xs font-medium text-zinc-800 dark:text-zinc-200 truncate">{{ $pdf['name'] }}</span>
                                        <span class="text-2xs text-zinc-400 dark:text-zinc-500">{{ round($pdf['size'] / 1024) }} KB</span>
                                    </a>
                                @endforeach
                            </div>
                        </div>
                    ...
                    @if ($post->tags && $post->tags->count() > 0)
                        <div class="rounded-2xl border border-zinc-200/50 bg-zinc-50/50 p-8 backdrop-blur-lg shadow-sm dark:border-zinc-800/50 dark:bg-zinc-900/30">
                            <h3 class="mb-5 flex items-center gap-3 text-sm font-bold uppercase tracking-widest text-zinc-900 dark:text-white">
                                <span class="h-2 w-2 bg-emerald-500"></span> Thématiques
                            </h3>
                    ...
                    <div class="rounded-2xl border border-zinc-200 bg-zinc-50/50 p-6 shadow-sm dark:border-zinc-800/50 dark:bg-zinc-900/30"
                        ...
                        <h3 class="mb-4 text-center text-sm font-semibold uppercase tracking-wider text-zinc-600 dark:text-zinc-400">Partager cet article</h3>
                        <div class="flex items-center justify-center gap-3">
                            <button @click="share('facebook')" class="flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-[#1877F2] hover:text-white hover:border-[#1877F2] dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400"><svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg></button>
                            <button @click="share('twitter')" class="flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-black hover:text-white hover:border-black dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400"><svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></button>
                            <button @click="share('linkedin')" class="flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-[#0A66C2] hover:text-white hover:border-[#0A66C2] dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400"><svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z"/></svg></button>
                            <button @click="share('whatsapp')" class="flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:bg-[#25D366] hover:text-white hover:border-[#25D366] dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400"><svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.52 3.449C18.24 1.245 15.24 0 12.045 0 5.463 0 .104 5.334.101 11.893c-.001 2.106.549 4.16 1.595 5.975L0 24l6.335-1.652c1.747.96 3.711 1.468 5.704 1.469h.004c6.58 0 11.939-5.336 11.943-11.893.002-3.181-1.238-6.173-3.466-8.475zm-8.476 19.333c-1.785-.001-3.536-.48-5.068-1.387l-.363-.214-3.766.982.999-3.655-.235-.373c-1.004-1.591-1.532-3.428-1.531-5.335.003-5.526 4.512-10.024 10.045-10.024 2.68 0 5.197 1.042 7.089 2.932 1.892 1.889 2.935 4.398 2.933 7.07-.003 5.523-4.512 10.004-10.003 10.004zm5.518-7.502c-.302-.15-1.789-.882-2.066-.983-.277-.1-.478-.15-.68.15s-.781.983-.957 1.183c-.176.2-.352.225-.654.075-1.921-.976-3.32-2.457-4.108-4.593-.075-.205.228-.182.523-.765.1-.2.05-.375-.025-.525s-.68-1.631-.932-2.233c-.246-.587-.496-.508-.68-.517-.176-.008-.377-.01-.579-.01-.2 0-.527.075-.803.375s-1.054 1.025-1.054 2.5 1.08 2.898 1.231 3.098c.15.2 2.115 3.208 5.12 4.49 2.112.903 2.87.777 3.931.625.816-.118 2.516-1.031 2.87-2.025.352-.993.352-1.844.247-2.025-.105-.175-.377-.275-.68-.425z"/></svg></button>
                            <button @click="copyLink()" class="relative flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:border-emerald-400 hover:bg-emerald-50 hover:text-emerald-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400">
                            ...
                            <a href="mailto:?subject={{ $shareTitle }}&body={{ $shareUrl }}" class="flex h-10 w-10 items-center justify-center border border-zinc-200 bg-white text-zinc-600 transition-colors hover:border-emerald-400 hover:bg-emerald-50 hover:text-emerald-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-400"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg></a>
                        </div>
                    </div>
                </div>
            </aside>
    ...
    @if (count($this->relatedPosts) > 0)
        <div class="border-t border-zinc-200 bg-zinc-50/40 py-8 sm:py-12 dark:border-zinc-800 dark:bg-zinc-950/50">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <h2 class="mb-6 sm:mb-8 text-xl sm:text-2xl font-bold text-zinc-900 dark:text-white">Articles similaires</h2>
                <div class="grid grid-cols-1 gap-4 sm:gap-7 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($this->relatedPosts as $related)
                        <a wire:navigate href="{{ route('posts.show', $related->slug) }}"
                            ...
                            <div class="relative overflow-hidden ring-1 ring-zinc-200 transition duration-500 ease-out group-hover:ring-emerald-300 dark:ring-zinc-700 dark:group-hover:ring-emerald-700">
                                @if ($related->hasMedia('featured'))
                                    <img src="{{ $related->getFirstMediaUrl('featured', 'card') }}" alt="{{ $related->title }}"
                                        class="aspect-video w-full object-cover transition duration-300 ease-out group-hover:scale-105" loading="eager" />
                                @else
                                    <div class="flex aspect-video w-full items-center justify-center bg-zinc-100 text-zinc-400 dark:bg-zinc-800">
                            ...
                            <div class="flex flex-1 flex-col gap-2 p-4">
                                <div class="relative transition duration-300 ease-out will-change-transform group-hover:translate-x-4.5">
                                    <div x-data="rotatingBadge()" class="absolute top-1/2 -left-4 -translate-y-1/2">
                                        <div class="translate-x-0.5 opacity-0 transition duration-300 ease-out will-change-transform group-hover:translate-x-0 group-hover:opacity-100">
        ...
        <section class="bg-white py-8 antialiased lg:py-16 dark:bg-zinc-950">
            <div class="mx-auto max-w-6xl px-4">
                <div class="mb-6 flex items-center justify-between">
                    ...
                    <span class="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-medium text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400">
